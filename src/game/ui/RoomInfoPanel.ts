...
export class RoomInfoPanel extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene) {
    ...
    this.add([panel, this.copy, upgrade, sell, target, this.evolveButton]);
    this.setDepth(220);
  ...
  refresh(room?: Room) {
    ...
    const signature = `${room.id}:${room.level}:${room.priority}:${room.activeFusion?.id ?? ''}:${room.mergedInto ?? ''}:${room.evolvedFusion}`;
