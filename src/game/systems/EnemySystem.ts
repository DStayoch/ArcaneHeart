...
export class EnemySystem {
  ...
  constructor(private scene: Phaser.Scene, private state: GameState, private economy: EconomySystem, private mutations: MutationSystem, private path: PathPoint[]) {}
  ...
  spawn(enemyId: EnemyId, elite = false) {
    const healthScale = 1 + Math.max(0, this.state.wave - 1) * 0.1;
    const speedScale = 1 + Math.max(0, this.state.wave - 1) * 0.018;
  ...
  private kill(enemy: Enemy) {
    enemy.alive = false;
    this.economy.addMana(enemy.def.rewardMana + (this.mutations.has('gets_hungry') && enemy.currentFloor() >= 7 ? 1 : 0));
